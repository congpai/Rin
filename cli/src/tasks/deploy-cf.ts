...
export function isSecretSyncVersionConflictError(stderr: string) {
  return (
    stderr.includes("[code: 10214]")
    || stderr.includes("latest version isn't currently deployed")
    || stderr.includes("latest version of your Worker isn't currently deployed")
    || stderr.includes("VERSION_NOT_DEPLOYED")
  );
}
...
async function syncWorkerSecrets(workerName: string) {
  if (process.env.SKIP_WORKER_SECRET_SYNC === "1" || process.env.SKIP_WORKER_SECRET_SYNC === "true") {
    console.log("ℹ️ SKIP_WORKER_SECRET_SYNC is set; skipping secret sync");
    return;
  }
  ...
  try {
    const result = await $`${bunExec} x wrangler secret bulk ${tempFile} --name ${workerName}`.quiet().nothrow();
    ...
    if (isSecretSyncVersionConflictError(`${stderr}\n${stdout}`)) {
      console.warn("⚠️ Worker secret sync skipped (Cloudflare 10214).");
      console.warn("   The Worker was deployed, but Wrangler could not attach secrets to the latest version.");
      console.warn("   Existing secrets in the Cloudflare dashboard are unchanged and still apply to production.");
      console.warn("   To sync from CI again: deploy the latest version in the dashboard, or run:");
      console.warn(`   wrangler versions secret bulk ${tempFile} --name ${workerName}`);
      return;
    }

    console.error("Failed to sync worker secrets");
    console.error(stripIndent(stdout));
    console.error(stripIndent(stderr));
    process.exit(1);
  } finally {
...
export function buildWranglerObservabilityConfig() {
  return stripIndent(`
    [observability]
    enabled = false
