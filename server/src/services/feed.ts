...
export function FeedService(): Hono<{
    ...
    app.delete('/:id', async (c) => {
        ...
        const relatedComments = await profileAsync(c, 'feed_delete_comments', () => db.query.comments.findMany({
            where: eq(comments.feedId, id_num),
        ...
        await profileAsync(c, 'feed_delete_storage_cleanup', () => cleanupUnreferencedImagesFromContents(db, env, contentsToScan));
...
export function SearchService(): Hono<{
    ...
    function paginateResults<T>(list: T[], page_num: number, limit_num: number) {
        if (list.length <= page_num * limit_num) {
            return { size: list.length, data: [], hasNext: false };
        }
        if (list.length <= page_num * limit_num + limit_num) {
            return {
                size: list.length,
                data: list.slice(page_num * limit_num),
                hasNext: false,
            };
        }
        return {
            size: list.length,
            data: list.slice(page_num * limit_num, page_num * limit_num + limit_num),
            hasNext: true,
        };
    }
    ...
    app.get('/:keyword', async (c) => {
        ...
        if (keyword === undefined || keyword.trim().length === 0) {
            return c.json({
                feeds: { size: 0, data: [], hasNext: false },
                moments: { size: 0, data: [], hasNext: false },
            });
        }

        const cacheKey = `search_v3_${keyword}`;
        const searchKeyword = `%${keyword}%`;
        const feedWhereClause = or(
        ...
        const cached = await profileAsync(c, 'feed_search_cache_db', () => cache.getOrSet(cacheKey, async () => {
            const [feedRows, momentRows] = await Promise.all([
                db.query.feeds.findMany({
                    where: admin ? feedWhereClause : and(feedWhereClause, eq(feeds.draft, 0)),
                    columns: admin ? undefined : { draft: false, listed: false },
                    with: {
                        hashtags: {
                            columns: {},
            ...
            const feedsResult = feedRows.map(({ content, hashtags, summary, ...other }: any) => {
                const avatar = extractImageWithMetadata(content);
                return {
        ...
        const feedPage = paginateResults(cached.feeds, page_num, limit_num);
        const momentPage = paginateResults(cached.moments, page_num, limit_num);

        return c.json({
            feeds: feedPage,
            moments: momentPage,
            hasNext: feedPage.hasNext || momentPage.hasNext,
        });
    });
    return app;
}
