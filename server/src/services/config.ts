...
import { isResendConfigured, sendResendEmail } from "../utils/resend";
import {
...
export function ConfigService(): Hono {
    ...
    app.post('/test-resend', async (c: AppContext) => {
        const admin = c.get('admin');
        ...
        const env = c.get('env');
        const body = await wrapTime(c, 'request_body', c.req.json()) as { to?: string };

        if (!isResendConfigured(env)) {
            return c.json({
                success: false,
                error: "Resend is not configured. Set RESEND_API_KEY and RESEND_FROM on the Worker.",
            }, 400);
        }

        const to = body.to?.trim();
        if (!to) {
            return c.json({ success: false, error: "Recipient email (to) is required" }, 400);
        }

        const result = await wrapTime(c, 'resend_send', sendResendEmail(env, {
            to,
            subject: "【二打六博客】Resend 测试邮件",
            html: "<p>如果你收到这封邮件，说明 Resend 配置正确。</p>",
        }));
    ...
    app.get('/compat-tasks/image-variants', async (c: AppContext) => {
        const admin = c.get('admin');
        ...
        return c.json(await wrapTime(c, 'compat_image_variants_list', listImageVariantBackfillCandidates(c.get('db'), c.get('env'))));
    });

    app.post('/compat-tasks/image-variants', async (c: AppContext) => {
        const admin = c.get('admin');
        ...
        if (keys.length > 10) {
            return c.text('At most 10 keys per request', 400);
        }
