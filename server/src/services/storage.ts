...
import { isValidVariantStorageKey } from "../utils/image-variants";
...
export function StorageService(): Hono {
    ...
    app.post('/', async (c: AppContext) => {
        ...
        const forcedStorageKey = typeof body.storageKey === "string" ? body.storageKey.trim() : "";
        ...
        const hashkey = forcedStorageKey
            ? forcedStorageKey
            : `${hash}.${suffix}`;

        if (forcedStorageKey && !isValidVariantStorageKey(forcedStorageKey)) {
        ...
        try {
            ...
            if (!forcedStorageKey && file.type?.startsWith("image/")) {
                const variantTask = profileAsync(c, "storage_variants", () => generateImageVariants(
                    env,
                    hashkey,
