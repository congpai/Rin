...
describe("image-variants", () => {
    it("builds predictable variant keys", () => {
        ...
        expect(buildVariantStorageKey(base, 640)).toBe(
            "images/abc123def4567890123456789012345678901234_w640.webp",
        );
    });

    it("validates forced variant storage keys", () => {
        expect(isValidVariantStorageKey("abc123def4567890123456789012345678901234_w640.webp")).toBe(true);
        expect(isValidVariantStorageKey("abc123_w640.webp")).toBe(false);
    });
    ...
    it("does not generate variants for variant keys", () => {
        expect(listVariantStorageKeys("images/abc123def4567890123456789012345678901234_w640.webp")).toEqual([]);
    });
});
