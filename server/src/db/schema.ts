...
export const momentHashtags = sqliteTable("moment_hashtags", {
    momentId: integer("moment_id").references(() => moments.id, { onDelete: "cascade" }).notNull(),
    hashtagId: integer("hashtag_id").references(() => hashtags.id, { onDelete: "cascade" }).notNull(),
    createdAt: created_at,
    updatedAt: updated_at,
...
export const momentHashtagsRelations = relations(momentHashtags, ({ one }) => ({
    moment: one(moments, {
        fields: [momentHashtags.momentId],
        references: [moments.id],
    }),
    hashtag: one(hashtags, {
        fields: [momentHashtags.hashtagId],
        references: [hashtags.id],
    }),
}));
