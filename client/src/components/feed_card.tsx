...
function FeedCardImage({ src, variant, priority = false }: { src: string; variant: FeedCardVariant; priority?: boolean }) {
    const canvasRef = useRef<HTMLCanvasElement>(null);
    const { src: cleanSrc, blurhash, width, height } = parseImageUrlMetadata(src);
    const responsive = buildResponsiveImageProps(cleanSrc, "(max-width: 768px) 100vw, 640px", 640);
...
export function FeedCard({ id, title, avatar, draft, listed, top, summary, hashtags, createdAt, updatedAt, preview = false, variant, imagePriority = false }: FeedCardProps) {
        ...
        <div className={styles.card}>
            {avatar ? (
                <div className={styles.imageWrap}>
                    <FeedCardImage src={avatar} variant={activeVariant} priority={imagePriority} />
                </div>
            ) : null}
