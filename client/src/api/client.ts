...
export interface CompatImageVariantCandidatesResponse {
  generatedAt: string;
  items: string[];
}

export interface CompatImageVariantRunResponse {
  processed: number;
  generated: number;
  skipped: number;
  failed: number;
...
class ConfigAPI {
  ...
  async getCompatImageVariantCandidates(): Promise<ApiResponse<CompatImageVariantCandidatesResponse>> {
    return this.http.get<CompatImageVariantCandidatesResponse>("/api/config/compat-tasks/image-variants");
  }
...
class StorageAPI {
  ...
  async upload(
    ...
    if (options?.storageKey) formData.append("storageKey", options.storageKey);
...
class SearchAPI {
  ...
  async search(keyword: string, params?: { page?: number; limit?: number }): Promise<ApiResponse<SearchResponse>> {
    const searchParams = new URLSearchParams();
    if (params?.page) searchParams.set("page", params.page.toString());
    if (params?.limit) searchParams.set("limit", params.limit.toString());
